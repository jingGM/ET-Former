...
class DeformCrossAttention(nn.Module):
    ...
    def __init__(self,
        ...
        super(DeformCrossAttention, self).__init__()

        self.init_cfg = init_cfg
        self.dropout = nn.Dropout(dropout)
        self.fp16_enabled = False
        self.deformable_attention = MSDeformableAttention3D(embed_dims=deformable_attention.embed_dims,
                                                            num_heads=8,
                                                            num_levels=deformable_attention.num_levels,
                                                            num_points=deformable_attention.num_points,
                                                            im2col_step=64,
                                                            dropout=0.1,
                                                            batch_first=True,
                                                            norm_cfg=None,
    ...
    def forward(self,
        ...
        count = torch.clamp(count, min=1.0).to(slots.device)

...
class TriplaneDownSample(BaseModel):
    def __init__(self, config, scale, data_type, model_type, device, sparse_embedding=True):
        ...
        if self.scale == 1:
            ...
            self.downsample_hw = nn.Sequential(nn.Conv2d(in_channels=config.cross.out_dim,
                                                         out_channels=config.cross.out_dim,
                                                         kernel_size=5, stride=2, padding=int((5 - 1) / 2)))
            self.downsample_hd = nn.Sequential(nn.Conv2d(in_channels=config.cross.out_dim,
                                                         out_channels=config.cross.out_dim,
                                                         kernel_size=5, stride=2, padding=int((5 - 1) / 2)))
            self.downsample_wd = nn.Sequential(nn.Conv2d(in_channels=config.cross.out_dim,
                                                         out_channels=config.cross.out_dim,
                                                         kernel_size=5, stride=2, padding=int((5 - 1) / 2)))
        ...
        self.cross_vox_size = config.cross.vox_size
    ...
    def _get_reference_points(self):
        x = torch.linspace(0.5, int(self.vox_size[0]) - 0.5, int(self.reference_num[0])).view(-1, 1, 1).expand(
            int(self.reference_num[0]), int(self.reference_num[1]), int(self.reference_num[2])) / self.vox_size[0]
        y = torch.linspace(0.5, int(self.vox_size[1]) - 0.5, int(self.reference_num[1])).view(1, -1, 1).expand(
            int(self.reference_num[0]), int(self.reference_num[1]), int(self.reference_num[2])) / self.vox_size[1]
        z = torch.linspace(0.5, int(self.vox_size[2]) - 0.5, int(self.reference_num[2])).view(1, 1, -1).expand(
            int(self.reference_num[0]), int(self.reference_num[1]), int(self.reference_num[2])) / self.vox_size[2]
    ...
    def _get_triplane_features(self, proposal=None, queries=None, vox_feats=None):
        ...
        # else:
        #     queries = queries[0]
        queries = queries[0].reshape(self.vox_size[0], self.vox_size[1], self.vox_size[2])
        queries = torch.from_numpy(np.asarray(np.where(queries > 0)))
        ...
        if self.scale == 1:
            hw_fts = self.downsample_hw(plane_hw.permute(2, 0, 1).unsqueeze(0))
            hd_fts = self.downsample_hd(plane_hd.permute(2, 0, 1).unsqueeze(0))
            wd_fts = self.downsample_wd(plane_wd.permute(2, 0, 1).unsqueeze(0))
        else:
            hw_fts = plane_hw.permute(2, 0, 1).unsqueeze(0)
            hd_fts = plane_hd.permute(2, 0, 1).unsqueeze(0)
            wd_fts = plane_wd.permute(2, 0, 1).unsqueeze(0)
        return hw_fts, hd_fts, wd_fts, queries
    ...
    def forward(self, img_feats, projection_matrix, pc_range, proposal, queries=None, vox_feats=None):
        ...
        plane_hw, plane_hd, plane_wd, indices = self._get_triplane_features(proposal=proposal, vox_feats=vox_feats,
                                                                            queries=queries)  # 1, C, H, W

        hw_self_fts = self.self_plan_hw_att(plane_hw)  # B, C, H, W
        hd_self_fts = self.self_plan_hd_att(plane_hd)  # B, C, H, W
        wd_self_fts = self.self_plan_wd_att(plane_wd)  # B, C, H, W
        ...
        if self.merge_type == MergeType.concat:
            voxels = torch.cat((cross_wd.unsqueeze(1).repeat(1, self.cross_vox_size[0], 1, 1, 1),
                                cross_hd.unsqueeze(2).repeat(1, 1, self.cross_vox_size[1], 1, 1),
                                cross_hw.unsqueeze(3).repeat(1, 1, 1, self.cross_vox_size[2], 1)), dim=-1) # B, H,W,D, C*3
        elif self.merge_type == MergeType.sum:

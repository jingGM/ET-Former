...
"""
import numpy as np
from utilities.configs import EvaluationNames
...
class SSCMetrics:
    def __init__(self, n_classes, only_iou=False):
        self.n_classes = n_classes
        self.only_iou = only_iou
        self.reset()
    ...
    def get_stats(self):
        ...
        if self.only_iou:
            return {EvaluationNames.precision: precision,
            EvaluationNames.recall: recall,
            EvaluationNames.iou_occ: iou}
        else:
            return {
                EvaluationNames.precision: precision,
                EvaluationNames.recall: recall,
                EvaluationNames.iou_occ: iou,
                EvaluationNames.iou_ssc: iou_ssc,
                EvaluationNames.iou_ssc_mean: np.mean(iou_ssc[1:]),
            }
